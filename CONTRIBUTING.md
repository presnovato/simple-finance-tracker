...
Перед коммитом полезно прогнать локальную проверку — она ищет токены
Telegram-бота, ключи OpenRouter и приватные ключи и печатает находки с
маскированными значениями:

```bash
python tools/check_secrets.py          # все файлы из git ls-files
python tools/check_secrets.py tma/dist # конкретные пути
```

Скрипт завершается с кодом 1 при находке. В CI ту же роль выполняет job
`secrets` с gitleaks по всей истории.
